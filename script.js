...
contactForm?.addEventListener('submit', async (e) => {
    ...
    try {
        // Try to load contact config
        let config = null;
        try {
            config = typeof contactConfig !== 'undefined' ? contactConfig : null;
        } catch (e) {
            console.log('No contact config found, using default behavior');
        }
        ...
        else if (config?.emailjs?.enabled && typeof emailjs !== 'undefined') {
            await emailjs.send(
        ...
        // If custom API is configured
        else if (config?.customApi?.enabled) {
            const response = await fetch(config.customApi.endpoint, {
                method: config.customApi.method,
                headers: config.customApi.headers,
        ...
        // Default behavior (no backend configured)
        else {
            // Simulate submission
            ...
            console.log('Form data (configure contact backend to actually send):', data);
        }
    } catch (error) {
        console.error('Form submission error:', error);
        showNotification('Oops! Something went wrong. Please email me directly at contact@devsecopssolution.in', 'error');
    } finally {
